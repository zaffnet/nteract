...
describe("comm reducers", () => {
  it("process comm_open and comm_message actions", () => {
    const state = new CommsRecord();
    const action = { type: "COMM_OPEN", target_module: "reducers", target_name: "setIn", data: { a: 2 }, comm_id: 100 };
    ...
    const actionMessage = { type: "COMM_MESSAGE", data: { path: ["a"], value: 3 }, comm_id: 100 };

    const afterMessageState = commsReducer(nextState, actionMessage);

    expect(afterMessageState
        .getIn(["models", 100])
        .toJS()).to.deep.equal({ a: 3 });
  });
  it("does a straight replacement for unknown comm messages", () => {
    ...
    const action = { type: "COMM_MESSAGE", data: { x: 5113, bagels: true }, comm_id: 101 };

    const nextState = commsReducer(state, action);

    expect(nextState
        .getIn(["models", 101])
        .toJS()).to.deep.equal({ bagels: true, x: 5113 });
  });
});
