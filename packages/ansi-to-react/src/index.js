...
export function ansiToInlineStyle(text) {
    return ansiToJSON(text).map(ansiJSONtoStyleBundle);
}
function linkifyBundle(bundle) {
    return Object.assign({}, bundle, { content: bundle.content.split(" ").reduce((result, word, index) => [
            ...result,
...
function Ansi(props) {
    return React.createElement("code", { className: props.className }, props.linkify
