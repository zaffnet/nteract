...
describe("Output", () => {
  it("handles display data", () => {
    const output = { output_type: "display_data", data: { "text/html": "<h1>Multiple</h1>", "text/plain": "<IPython.core.display.HTML object>" }, metadata: {} };
  ...
  it("handles execute_component", () => {
    const output = { data: { "text/html": ['<img src="https://avatars2.githubusercontent.com/u/12401040?v=3&s=200"/>'], "text/plain": ["<IPython.core.display.Image object>"] }, execution_count: 7, metadata: {}, output_type: "execute_result" };
  ...
  it("handles errors/tracebacks", () => {
    const output = { output_type: "error", traceback: ["whoa there buckaroo!"], ename: "BuckarooException", evalue: "whoa!" };

    const component = shallow(<Output output={output} />);
    expect(component.type()).toEqual(Ansi);

    const outputNoTraceback = { output_type: "error", ename: "BuckarooException", evalue: "whoa!" };
